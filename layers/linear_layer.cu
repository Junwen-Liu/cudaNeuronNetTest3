...
//forward propogation will output Z(Z=W*A+b)
__global__ void linearLayerForward( float* W, float* A
	, float* Z, float* b,
...
//compute dA is the error introduced by input A, dA will be passed to the preceding layer
...
//dW is error that introduced by W and use to update W
...
//db is the error introduced by bias and db is used to update bias, where dZ(i) is ith column of dZ(ith input in a batch) and m is size of a batch.
...
//initialize weight matrix with values from normal distribution with 0 mean and standard deviation equal to 1.
